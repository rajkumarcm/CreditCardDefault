...
5. What measures can be taken to mitigate the risks of insolvency?
...
**Explanation for notable variables**  
...
bs | bill statement
ap | amount paid
default_r | Whether the client has defaulted (response variable)  


**Repayment Status Level Description**  
...
1..8 | Represents the number of months due
...
```{r age_default, fig.height=5, fig.width=12, results='markup'}
...
The `t.test` on age between those who defaulted on their credit card bills and those who did not, returned a `p.value` of `r ttest.age_default$p.value`. In simple words, the difference in age between the two subgroups is not equal to 0.  
...
```{r, fig.height=5, fig.width=12, results='markup'}
...
### Education and Amount Paid in September  
```{r, results='markup'}
educ_ap_sep <- aov(ap_sep~education, data=data)
summary(educ_ap_sep)
```

### Education and Amount Paid in July
```{r, results='markup'}
educ_ap_july <- aov(ap_july~education, data=data)
summary(educ_ap_july)
```

Amount paid by people with different qualification differs on average, as per the ANOVA results. In other words we choose Alternative hypothesis that the mean of amount paid by different subgroups are not the same.  

## Marital and Default

```{r marital_default, results='markup'}
# Marital status (1 = married; 2 = single; 3 = others)
...
## Marital and Amount paid in September  
```{r, results='markup'}
marital_ap_sep <- aov(ap_sep~marital, data=data)
summary(marital_ap_sep)
```

## Marital and Amount paid in July  
```{r, results='markup'}
marital_ap_july <- aov(ap_july~marital, data=data)
summary(marital_ap_july)
```

The ANOVA test results show that the amount paid by Married, Single and Others are different on average.  
...
From the above plot, it can be understood that there are more number of Females who default compared to Males. Why is this the case ? A possible cause could be a bias in education. Let's understand this further. Also there are more number of Females who have not defaulted on their bill.  
...
I wish not to select features based on the regsubsets plot as I strongly believe it misses out on many important variables that play critical role in predicting whether a person would default on his/her next month bill.  
...
Regarding the gender and education, I believe some of the visual plots and initial conclusions made in the EDA were skewed by the SMOTE process.  
...
```{r, results='markup'}
tr_pred <- full_model$fitted.values
tr_pred <- ifelse(tr_pred > 0.37, 'Yes', 'No') # I got this value after the following steps
tr_pred <- factor(tr_pred)
confusionMatrix(tr_pred, reference = train_set$default_r)
```
...
get_risky_amt <- function(cm, t_data)
{
  ...
  # depending on the subset (in k-fold) the there will be variation in prediction. 
  ...
  total_bill <- colSums(test_subset[, 13:18]) # bs for all months
  total_paid <- colSums(test_subset[, 19:24]) # ap for all months
...
roc(test_set$default_r, fm.y_prob,
            smoothed = TRUE,
            plot=TRUE, grid=TRUE,
...
```{r dt_rules, results='markup'}
rmarkdown::paged_table(rpart.rules(dt_fit, cover=T))
```
...
```{r}
var_importance <- dt_fit$variable.importance
...
Now if you recall what I mentioned in the feature selection section and contrast this plot with that, my decision would make sense.  

**Confusion Matrix**  

```{r eval_dt, results='markup'}
...
Adaboost appears to take over 2 hours to train and due to time constraints, I wish not to proceed further with this model.  

### Random Forest
```{r rf, results='markup'}
library(randomForest)

train_lbls <- train_set$default_r
train_set3 <- train_set[, c(-1, -25, -26)] # id, default_r, age_group
test_lbls <- test_set$default_r
test_set3 <- test_set[, c(-1, -25, -26)]

rfFit <- randomForest(x=train_set3, y=train_lbls, importance=T)
rfFit
```

```{r, results='markup'}
rf_pred <- predict(rfFit, test_set)
confusionMatrix(rf_pred, reference=test_lbls)
```

Try changing the class weights
```{r, results='markup'}
weights <- train_lbls
weights <- ifelse(train_lbls=='Yes', 0.7, 0.3)
rfFit2 <- randomForest(x=train_set3, y=train_lbls, importance=T, weights=weights)

```

**Confusion Matrix** on Train set
```{r, results='markup'}
rf_pred2_tr <- predict(rfFit2, train_set3)
confusionMatrix(rf_pred2_tr, reference=train_lbls)
```

**Confusion Matrix** on Test set
```{r, results='markup'}
rf_pred2_ts <- predict(rfFit2, test_set)
confusionMatrix(rf_pred2_ts, reference=test_lbls)
```

Comparing the metrics on both Training and Testing sets, I can not see any sign of overfitting. Hence I wish not to prune or alter the depth of the tree that acts as a regularization.  

The random forest after adjusting the class weights gives slightly better results, and due to time constraints I wish to end my research here. The adaboost code is in the Rmd file and it appears to take over 2 hours to complete. It is not feasible to re-run for every single change we make to adaboost code. Hence I have decided to comment it.  

My conclusion regarding the modeling is, Decision Trees are great in coming up with good set of rules, and therefore they are much better compared to Logistic Regression and KNN, at least the case with this dataset. 
